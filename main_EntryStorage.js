...
function addEntry(content) {
    if (typeof content !== 'string' || content.length === 0) {
        ...
        // Optionally throw an error here too if that's desired behavior for invalid input
        // throw new Error("addEntry: Content must be a non-empty string.");
    ...
    if (newData.length > QR_CHAR_LIMIT) {
        ...
        throw new Error(errorMessage); // Throw error and stop execution
        // The 'return' statement below is now unreachable due to 'throw', but kept for clarity if 'throw' was conditional.
        // return; 
    }
